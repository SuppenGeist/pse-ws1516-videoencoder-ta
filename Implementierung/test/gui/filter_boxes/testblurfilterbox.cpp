...
void TestBlurFilterBox::testChangeIntensity() {
	...
	QVERIFY( false == QObject::connect(slider, SIGNAL(valueChanged(int)), filterbox_,
	                                   SLOT(sliderChanged(int)), Qt::UniqueConnection).operator RestrictedBool()  );
	...
	QVERIFY( false == QObject::connect(spinBox,SIGNAL(valueChanged(int)),filterbox_,
	                                   SLOT(spinBoxChanged(int)), Qt::UniqueConnection).operator RestrictedBool() );
}
