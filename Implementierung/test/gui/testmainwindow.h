...
class WriteYuvFilePath : public QThread {
  ...
  public:
	...
	void setHeight(int height) {
		height_ = QString::number(height);
	}
	void setWidth(int width) {
		width_ = QString::number(width);
	}
	void setCompression(Utility::Compression compression) {
		compression_ = compression;
	}
	void setYuvType(Utility::YuvType type) {
		type_ = type;
	}
	void setFps(int fps) {
		fps_=QString::number(fps);
	}
  private:
	void run() {
		QTest::qSleep(500);
		...
		QTest::qSleep(500);
		QApplication::postEvent(okButton, new QMouseEvent(QEvent::MouseButtonPress,QPoint(1,1),
		                        Qt::LeftButton,Qt::LeftButton,Qt::KeyboardModifier::NoModifier));
		QApplication::postEvent(okButton, new QMouseEvent(QEvent::MouseButtonRelease,QPoint(1,1),
		                        Qt::LeftButton,Qt::LeftButton,Qt::KeyboardModifier::NoModifier));
		...
		for(int i = 0; i < lineEditList.length(); i++) {
			...
			if(lineEditList.at(i)->objectName() == QString("lineEdit_width_") ) {
				QApplication::postEvent(lineEditList.at(i),new QKeyEvent(QEvent::KeyPress,Qt::Key_A,Qt::NoModifier,
				                        width_));
			} else if(lineEditList.at(i)->objectName()== QString("lineEdit_height_")) {
				QApplication::postEvent(lineEditList.at(i),new QKeyEvent(QEvent::KeyPress,Qt::Key_A,Qt::NoModifier,
				                        height_));
			} else  if(lineEditList.at(i)->objectName()==QString("lineEdit_fps_")) {
				QApplication::postEvent(lineEditList.at(i),new QMouseEvent(QEvent::MouseButtonDblClick,QPoint(5,5),
		...
		case Utility::YuvType::YUV444 :
			QApplication::postEvent(comboBox,new QKeyEvent(QEvent::KeyPress,Qt::Key_Down,Qt::NoModifier));
