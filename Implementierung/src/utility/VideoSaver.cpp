...
void Utility::VideoSaver::saveVideo(Model::Video* video, QString filename) {
	...
	for(unsigned int i = 0; i < video_->getNumberOfFrames(); i++) {
		...
		struct SwsContext* swsContext = sws_getContext(c->width, c->height,AVPixelFormat::AV_PIX_FMT_RGB24,
		                                c->width, c->height, AVPixelFormat::PIX_FMT_YUV420P, SWS_FAST_BILINEAR, NULL, NULL, NULL);
		sws_scale(swsContext, video_->getFrame(i)->data, video_->getFrame(i)->linesize, 0, c->height,
