...
void Utility::RGBDifferenceCalculator::calculateP()
{
    ...
    std::size_t i=0;
    do {
        for(; i<(video_->getNumberOfFrames()>referenceVideo_->getNumberOfFrames()?referenceVideo_->getNumberOfFrames():video_->getNumberOfFrames())&&isRunning_; i++) {
            QImage *currentFrame = video_->getFrame(i);
            QImage *refCurrentFrame = referenceVideo_->getFrame(i);
            auto outFrame = std::make_unique<QImage>(currentFrame->width(), currentFrame->height(), currentFrame->format());
