...
unique_ptr<Model::AVVideo> VideoLoader::loadVideo() {
    ...
    if(path != NULL && !path.isEmpty() && avformat_open_input(&formatContext, path.toUtf8(), NULL, NULL)!=0)
    ...
    AVFrame *frame = NULL;
    unique_ptr<Model::AVVideo> video = make_unique<Model::AVVideo>(codecContext->framerate.num, codecContext->width, codecContext->height);
